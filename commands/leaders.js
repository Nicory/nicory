...
const lvlUtils = require('../utils/levelsUtils.coffee');
...
module.exports = {
  ...
  execute: async (message, args, client) => {
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription("Этот рейтинг базируется на кол-ве опыта, которое получили участники за общение на этом сервере.\nОпыт выдается рандомно: от 10 до 25 единиц за сообщение.")
      .setColor(0xe155ff)
      .setThumbnail(message.guild.iconURL());

    const exps = new Map()

    for (let i of message.guild.members.cache.array()) { 
      let exp = await db.get(`${message.guild.id}_${i.user.id}`, "exp", 0);
    ...
    sorted.forEach((k, v) => { 
      ...
      embed.addField(member.user.tag, `EXP: ${k}; LVL: ${lvlUtils.getLevelFromExp(k)}`);
    });

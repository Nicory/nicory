module.exports = {
  ...
  async execute(message, args, client){
    ...
    let authorBalance = await db.get(`${message.guild.id}_${message.author.id}`, 'money', parseInt(0));
    let memberBalance = await db.get(`${message.guild.id}_${user.id}`, 'money', parseInt(0));
    ...
    if (rand==0) {
      ...
      // Баланс мембера
      let stealedBalance = memberBalance - rand2
      await db.set(`${message.guild.id}_${user.id}`, 'money', stealedBalance);

      // Баланс автора
      let newBalance = authorBalance + rand2
      await db.set(`${message.guild.id}_${message.author.id}`, 'money', newBalance);

    } else if (rand==1) {

      if (proeb==0) {
        return message.reply(`вас заметили и вызвали полицию, вы были задержаны!`)
      }

      // Баланс автора
      let newBalance = authorBalance - proeb
      await db.set(`${message.guild.id}_${message.author.id}`, 'money', newBalance);

      message.reply(`вас заметили и вызвали полицию, с вашего счета было снято ${proeb} кредитов!`)  
    }
